...
function Detect-CsvDelimiter {
    <#
        ...
        Teste en ordre de priorité : ; (France) → , (US) → TAB → espace
    ...
    try {
        ...
        # Tester les délimiteurs courants en ordre de priorité
...
$btnCsv.Add_Click({
    ...
    $delimiterName = $(if ($delimiter -eq ';') { 'Point-virgule' } elseif ($delimiter -eq ',') { 'Virgule' } elseif ($delimiter -eq "`t") { 'TAB' } else { 'Espace' })
    Write-Log "✓ Délimiteur détecté : $delimiterName"
    ...
    try   { $script:donnees = @(Import-Csv -Path $ofd.FileName -Delimiter $delimiter -Encoding UTF8) }
        ...
        try {
            ...
            Write-Log "ERREUR CRITIQUE : Impossible de lire le CSV même avec délimiteur détecté ($delimiterName)"
            Write-Log "Message d'erreur : $($_.Exception.Message)"
            return
        }
    }
    ...
    Write-Log "✓ CSV chargé : $($script:donnees.Count) ligne(s) - Délimiteur : $delimiterName - Colonnes : $($colonnes.Count)"

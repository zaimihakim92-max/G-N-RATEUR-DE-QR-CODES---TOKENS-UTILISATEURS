...
function Invoke-Simulation {
    ...
    try {
        ...
        foreach ($res in (Get-CMResource -ResourceType System -Fast -ErrorAction SilentlyContinue | Where-Object { $_.Name })) {
            ...
            if (-not $devIndex.ContainsKey($k)) { $devIndex[$k] = New-Object System.Collections.Generic.List[int] }
            [void]$devIndex[$k].Add([int]$res.ResourceId)
        }
        ...
        foreach ($name in $names) {
            ...
            $rids = if ($devIndex.ContainsKey($k)) { $devIndex[$k] } else { @() }
            if ($rids.Count -eq 0) {
            ...
            elseif ($rids.Count -gt 1) {
                $ridList = $rids -join ','
                $data.Add([pscustomobject]@{ Name=$name; Category='AMBIGU'; ResourceID=$null; Detail="ResourceID multiples : $ridList" })
                Write-Log "AMBIGU : '$name' -> $($rids.Count) enregistrements ($ridList). IGNORE." 'WARN'
...
function Confirm-ByTyping {
    ...
    $f.Add_Shown({ $t.Focus() }.GetNewClosure())
...
function Show-CollectionMembers {
    ...
    try {
        ...
        Write-Log "Erreur lecture des membres : $($_.Exception.Message)" 'ERR'
    }
    finally {
        Set-Location $prev -ErrorAction SilentlyContinue
    ...
    # Projection defensive des proprietes utiles.
    ...
    $w.Size = New-Object System.Drawing.Size(760,560)
    $w.MinimumSize = New-Object System.Drawing.Size(560,400)
    $w.StartPosition = 'CenterParent'
    $w.Font = New-Object System.Drawing.Font('Segoe UI',9)

    $lblInfo = New-Object System.Windows.Forms.Label
    $lblInfo.SetBounds(12,10,540,20); $lblInfo.Anchor = 'Top,Left,Right'

    $lblFilter = New-Object System.Windows.Forms.Label
    $lblFilter.Text = "Filtre :"; $lblFilter.SetBounds(12,36,45,22)
    $txtFilter = New-Object System.Windows.Forms.TextBox
    $txtFilter.SetBounds(60,33,400,24); $txtFilter.Anchor = 'Top,Left,Right'

    $g = New-Object System.Windows.Forms.DataGridView
    $g.SetBounds(12,66,724,440); $g.Anchor = 'Top,Left,Right,Bottom'
    $g.ReadOnly = $true; $g.AllowUserToAddRows = $false; $g.RowHeadersVisible = $false
    $g.SelectionMode = 'FullRowSelect'; $g.AutoSizeColumnsMode = 'Fill'
    ...
    $btnCsv.Text = "Extraire CSV"; $btnCsv.SetBounds(520,514,110,32); $btnCsv.Anchor = 'Bottom,Right'
    $btnClose = New-Object System.Windows.Forms.Button
    $btnClose.Text = "Fermer"; $btnClose.SetBounds(636,514,100,32); $btnClose.Anchor = 'Bottom,Right'; $btnClose.DialogResult = 'Cancel'

    $w.Controls.AddRange(@($lblInfo,$lblFilter,$txtFilter,$g,$btnCsv,$btnClose))

    # Rendu (avec filtre) piloté par la liste $rows capturée en closure.
    $render = {
        param($flt)
        $g.Rows.Clear()
        $shown = 0
        foreach ($r in $rows) {
            ...
            $idx = $g.Rows.Add($r.Name, $r.ResourceID, $r.Domaine, $r.OS, $r.Appartenance)
            if ($r.Appartenance -eq 'Direct') {
                $g.Rows[$idx].DefaultCellStyle.BackColor = [System.Drawing.Color]::FromArgb(220,235,250)
            }
            $shown++
        }
        $lblInfo.Text = "$($rows.Count) membre(s) - direct : $($directSet.Count) - affiche(s) : $shown"
    }.GetNewClosure()

    & $render $null
    $txtFilter.Add_TextChanged({ & $render $txtFilter.Text.Trim() }.GetNewClosure())
    ...
    $btnCsv.Add_Click({
        ...
        if ($sfd.ShowDialog() -eq 'OK') {
            try {
                $rows | Select-Object Name,ResourceID,Domaine,OS,Appartenance |

...
function Show-CollectionMembers {
    ...
    $render = {
        ...
        foreach ($r in $rows) {
            ...
            $vals.Add($r.Name); $vals.Add($r.ResourceID); $vals.Add($r.Domaine); $vals.Add($r.OS); $vals.Add($r.Appartenance)
    ...
    $btnAddCol.Add_Click({
        ...
        $already = @('Name','ResourceID','Domain','DeviceOS') + ($extraCols | ForEach-Object { $_.Prop })
        ...
        if ($sfd.ShowDialog() -eq 'OK') {
            try {
                $out = foreach ($r in $rows) {
                    $o = [ordered]@{ Name=$r.Name; ResourceID=$r.ResourceID; Domaine=$r.Domaine; OS=$r.OS; Appartenance=$r.Appartenance }
                    foreach ($ec in $extraCols) {
                        $v = $null
                        try { $v = $r.Member.$($ec.Prop) } catch { }
                        if ($v -is [array]) { $v = ($v -join '; ') }
    ...
    $btnRemoveSel.Add_Click({
        ...
        # L'etat a change : on invalide la simulation principale (verrou coherent).
        $script:SimulationDone = $false
        Update-ButtonsState
